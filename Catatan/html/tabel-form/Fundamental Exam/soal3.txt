...
var funBuy = (dataId) => {
    ...
    // if (produkTerpilih.stock == 0) {
    //     alert(`Stock ${produkTerpilih.name} kurang ${produkTerpilih.stock}`);
    // }
    // else {
    //     var cekId = arrCart.findIndex((val) => {
    //        return val.id == dataId
    //     });
    //     arrCart[cekId].qty ++;
    // }
    ...
    if (cekCart == undefined) {
        produkTerpilih.stock = produkTerpilih.stock - 1;
        arrCart.push({ ...produkTerpilih, qty: 1 });
    }
    else {
        if (produkTerpilih.stock == 0) {
            alert(`Stock ${produkTerpilih.name} Habis`);
    ...
    console.log(produkTerpilih.stock);
